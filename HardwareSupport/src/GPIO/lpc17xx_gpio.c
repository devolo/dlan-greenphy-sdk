...
/*********************************************************************//**
 * @brief		Helper function to check the byteNum used.
 * @param[in]	byteNum		Byte part number, should be in range from 0 to 3
 * @return		0 ERROR
 *              1 OK
 *
 * Note: For the GreenPHY LPCxpresso GCC build, uint8_t is unsigned and the
 * check for byteNum >= 0 causes a compiler warning, so UINT8_T_IS_NOT_UNSIGNED
 * is not defined. But this is not automatically checked, there are no
 * Autotools involved.
 *
 * So, beware, if you are using an other build environment ...
 **********************************************************************/
static int FIO_byteNumOk(uint8_t byteNum)
{
	int rv=0;

#ifdef UINT8_T_IS_NOT_UNSIGNED
